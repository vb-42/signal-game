    ...
    >
      ...
      >
        ...
        >
          ...
          <p className="text-sm text-gray-400 mt-0.5">{signal.description}</p>
        </div>
      </div>
    </div>
  );
...
export interface SignalCarouselHandle {
  next: () => void;
  prev: () => void;
}
...
interface SignalCarouselProps {
  className?: string;
  ref?: Ref<SignalCarouselHandle>;
}

const SignalCarousel = forwardRef<SignalCarouselHandle, SignalCarouselProps>(
  function SignalCarousel({ className = "" }, ref) {
    ...
    const handlePointerMove = useCallback(
      (e: PointerEvent<HTMLDivElement>) => {
        ...
        scrollRef.current.target =
          dragRef.current.startTarget + distance * SCROLL_SPEED;
      },
      [],
    );
    ...
    const cards = SIGNALS.map((signal, i) => {
      ...
      const radius =
        (GALLERY_HALF_WIDTH * GALLERY_HALF_WIDTH +
          GALLERY_BEND * GALLERY_BEND) /
        (2 * GALLERY_BEND);
      const arc =
        radius -
        Math.sqrt(Math.max(0, radius * radius - effectiveX * effectiveX));
      const rotation =
        Math.sign(x) * Math.asin(effectiveX / radius) * (180 / Math.PI);
      const distance = Math.abs(offset);
      const scale = 1 - Math.min(distance * 0.08, 0.36);
      const opacity =
        1 - Math.min(Math.max(distance - 1.5, 0) * 0.22, 0.72);
    ...
    const frontIndex =
      ((Math.round(scrollPosition) % CARD_COUNT) + CARD_COUNT) % CARD_COUNT;
    ...
    return (
      ...
      >
        ...
        <div
          ...
          style={{
            background:
              "linear-gradient(to top, rgba(8,8,16,1) 0%, transparent 100%)",
          }}
        />
      </div>
    );
